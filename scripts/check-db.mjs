...
const DATABASE_URL = process.env.DATABASE_URL || (process.env as any).DATABASE_URL;
...
async function main() {
  ...
  try {
    ...
    const dlCols = await db`SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'daily_logs' ORDER BY ordinal_position`;
    ...
    const wr = await db`SELECT to_regclass('weekly_reviews') as exists`;
    console.log('weekly_reviews exists:', wr[0]?.exists);

    // Check goals
    const gl = await db`SELECT to_regclass('goals') as exists`;
    console.log('goals exists:', gl[0]?.exists);

    // Check onboarding_profiles
    const op = await db`SELECT to_regclass('onboarding_profiles') as exists`;
    console.log('onboarding_profiles exists:', op[0]?.exists);

    // Check coach_lessons
    const cl = await db`SELECT to_regclass('coach_lessons') as exists`;
    console.log('coach_lessons exists:', cl[0]?.exists);

    // Check user_context
    const uc = await db`SELECT to_regclass('user_context') as exists`;
    console.log('user_context exists:', uc[0]?.exists);

    // Check energy_log
    const el = await db`SELECT to_regclass('energy_log') as exists`;
    console.log('energy_log exists:', el[0]?.exists);

    // Try getting current user
    const users = await db`SELECT id, email, organization_id FROM users LIMIT 5`;
    console.log('=== users in DB ===');
    users.forEach((u: any) => console.log(JSON.stringify(u)));

    // Check onboarding_profiles data
    try {
      const profiles = await db`SELECT user_id, completed, profile FROM onboarding_profiles LIMIT 5`;
