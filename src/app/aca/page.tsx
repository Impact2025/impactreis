      ...
      <div className="min-h-screen bg-white pb-28">
        <div className="sticky top-0 z-10 bg-white border-b border-line">
          <div className="max-w-lg mx-auto px-5 py-4 flex items-center gap-3">
            ...
            <div>
              <h1 className="text-[17px] font-semibold text-ink">ACA Herstelpad</h1>
              <p className="text-[11px] text-ink-soft">7 weken naar herstel</p>
            </div>
          </div>
        </div>

        <div className="max-w-lg mx-auto px-5 pt-8">
          <div className="rounded-[20px] bg-surface-inverse p-6 mb-6">
          ...
          <div className="space-y-3 mb-8">
            {[
              ...
              <div key={item.icon} className="flex items-start gap-3 rounded-[14px] bg-surface-sunken p-4">
                <span className="text-xl">{item.icon}</span>
                <div>
                  <p className="text-[13px] font-semibold text-ink">{item.title}</p>
                  <p className="text-[12px] text-ink-soft mt-0.5">{item.desc}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <button
            onClick={handleStart}
            className="w-full py-4 bg-primary text-white text-[15px] font-bold rounded-[16px] active:scale-[0.98] transition-transform shadow-[0_4px_20px_rgba(81,96,80,0.3)]"
          >
  ...
  return (
    <div className="min-h-screen bg-white pb-28">
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-line">
        <div className="max-w-lg mx-auto px-5 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-[17px] font-semibold text-ink">ACA Herstelpad</h1>
              <p className="text-[11px] text-ink-soft">Dag {dayCount} van je reis</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-[11px] font-bold text-primary tabular-nums">WEEK {currentWeek}/7</p>
            <p className="text-[10px] text-ink-soft">{week.title}</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto px-5 pt-5 space-y-4">
        ...
        {/* Belofte van de week */}
        <div className="rounded-[16px] bg-primary-muted border border-primary-light px-5 py-4">
          <p className="text-[9px] font-bold text-primary uppercase tracking-[0.18em] mb-1.5">Belofte van de week</p>
          <p className="text-[15px] font-semibold text-ink italic leading-snug">
        ...
        <div className="flex items-center gap-3">
          ...
          <p className="text-[11px] font-semibold text-ink-soft tabular-nums shrink-0">{done3}/3 vandaag</p>
        </div>
        ...
        <div className="rounded-[16px] border border-line bg-white overflow-hidden">
          ...
          >
            <div className={`w-9 h-9 rounded-[10px] flex items-center justify-center ${dayData.morning ? 'bg-primary-muted' : 'bg-tertiary-soft'}`}>
              {dayData.morning
                ? <CheckCircle size={17} className="text-primary" />
                : <Sun size={17} className="text-tertiary" />}
            </div>
            <div className="flex-1 text-left">
              <p className="text-[13px] font-semibold text-ink">Ochtend · Liefdevolle Ouder</p>
              <p className="text-[11px] text-ink-soft">
                {dayData.morning ? 'Voltooid ✓' : '5 minuten · Verbinding maken'}
              </p>
            </div>
            <ChevronRight size={15} className={`text-ink-soft transition-transform duration-200 ${morningOpen ? 'rotate-90' : ''}`} />
          </button>

          {morningOpen && (
            <div className="px-4 pb-4 border-t border-surface-sunken space-y-3">
              <div className="rounded-[12px] bg-surface-inverse p-4 mt-3">
              ...
              <div className="rounded-[12px] bg-primary-muted border border-primary-light p-3">
                <p className="text-[10px] font-bold text-primary uppercase tracking-widest mb-1">Belofte van deze week</p>
                <p className="text-[13px] text-ink italic">&ldquo;{week.belofte}&rdquo;</p>
              </div>
              {!dayData.morning && (
                <button
                  onClick={handleMorningDone}
                  className="w-full py-3 bg-primary text-white text-[14px] font-semibold rounded-[12px] active:scale-[0.98] transition-transform shadow-[0_2px_8px_rgba(81,96,80,0.25)]"
                >
                  Gedaan ✓
                </button>
              )}
        ...
        <div className="rounded-[16px] border border-line bg-white overflow-hidden">
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-4"
          >
            <div className={`w-9 h-9 rounded-[10px] flex items-center justify-center ${dayData.halt.length > 0 ? 'bg-primary-muted' : 'bg-accent-soft'}`}>
              {dayData.halt.length > 0
                ? <CheckCircle size={17} className="text-primary" />
                : <Coffee size={17} className="text-accent" />}
            </div>
            <div className="flex-1 text-left">
              <p className="text-[13px] font-semibold text-ink">Middag · HALT Check</p>
              <p className="text-[11px] text-ink-soft">
                {dayData.halt.length > 0
            ...
            <ChevronRight size={15} className={`text-ink-soft transition-transform duration-200 ${haltOpen ? 'rotate-90' : ''}`} />
          </button>

          {haltOpen && (
            <div className="px-4 pb-4 border-t border-surface-sunken">
              <p className="text-[12px] text-ink-soft mt-3 mb-3">Ben je op dit moment...?</p>
                  ...
                  return (
                    <button
                      ...
                      className={`p-3 rounded-[12px] border text-left transition-all active:scale-[0.97] ${
                        sel ? 'border-primary bg-primary-muted' : 'border-line bg-white'
                      }`}
                    >
                      <span className="text-lg">{item.emoji}</span>
                      <p className={`text-[12px] font-semibold mt-1 ${sel ? 'text-primary' : 'text-ink'}`}>
                        {item.label}
                      </p>
                    </button>
                  );
              ...
              {dayData.halt.length > 0 && (
                <div className="space-y-2 mb-3">
                  {HALT_ITEMS.filter(i => dayData.halt.includes(i.key)).map(item => (
                    <div key={item.key} className="rounded-[12px] bg-surface-sunken p-3">
                      <p className="text-[11px] font-bold text-ink mb-0.5">{item.emoji} {item.label}</p>
                      <p className="text-[12px] text-ink-soft leading-relaxed">{item.advice}</p>
                    </div>
                  ))}
                </div>
              )}
              ...
              <button
                ...
                className="w-full py-3 bg-surface-inverse text-white text-[14px] font-semibold rounded-[12px] active:scale-[0.98] transition-transform"
              >
        ...
        <div className="rounded-[16px] border border-line bg-white overflow-hidden">
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-4"
          >
            <div className={`w-9 h-9 rounded-[10px] flex items-center justify-center ${dayData.eveningSaved ? 'bg-primary-muted' : 'bg-tertiary-soft'}`}>
              {dayData.eveningSaved
                ? <CheckCircle size={17} className="text-primary" />
                : <Moon size={17} className="text-tertiary" />}
            </div>
            <div className="flex-1 text-left">
              <p className="text-[13px] font-semibold text-ink">Avond · Stap 10 Inventaris</p>
              <p className="text-[11px] text-ink-soft">
                {dayData.eveningSaved ? 'Voltooid ✓' : '5 minuten · Goedkeuring geven'}
              </p>
            </div>
            <ChevronRight size={15} className={`text-ink-soft transition-transform duration-200 ${eveningOpen ? 'rotate-90' : ''}`} />
          </button>

          {eveningOpen && (
            <div className="px-4 pb-4 border-t border-surface-sunken">
              <p className="text-[12px] text-ink-soft mt-3 mb-3 leading-relaxed">
                Schrijf 2 dingen waarvoor je jezelf vandaag <span className="font-semibold text-ink">goedkeuring</span> geeft.
                Niet waarvoor je trots bent — goedkeuring. Er is een verschil.
              </p>
              <div className="space-y-3 mb-3">
                {[0, 1].map(i => (
                  <div key={i} className="flex items-center gap-2.5">
                    <span className="w-6 h-6 rounded-full bg-primary-muted text-primary text-[11px] font-bold flex items-center justify-center flex-shrink-0">
                      {i + 1}
                    </span>
                    <input
                      type="text"
              ...
              <button
                ...
                className="w-full py-3 bg-primary text-white text-[14px] font-semibold rounded-[12px] active:scale-[0.98] transition-transform disabled:opacity-40 shadow-[0_2px_8px_rgba(81,96,80,0.25)]"
              >
                Opslaan ✓
              </button>
            </div>
        ...
        {/* ── WEEK THEMA ─────────────────────────────────────────── */}
        <div className="rounded-[20px] bg-surface-inverse p-5">
          <div className="flex items-center gap-2.5 mb-4">
            <span className="text-xl">{week.icon}</span>
            <div>
              <p className="text-[9px] font-bold text-primary uppercase tracking-[0.18em]">
        ...
        {/* ── 7-WEEK TIMELINE ────────────────────────────────────── */}
        <div>
          <p className="text-[13px] font-bold text-ink mb-3">Jouw herstelreis</p>
              ...
              return (
                <div
                  ...
                  className={`flex-none w-[88px] snap-start rounded-[14px] p-3 ${
                    current ? 'bg-surface-inverse border-2 border-primary' :
                    past    ? 'bg-primary-muted border border-primary-light' :
                              'bg-surface-sunken border border-line'
                  }`}
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-base">{w.icon}</span>
                    {past    && <CheckCircle size={11} className="text-primary" />}
                    {current && <span className="text-[8px] font-bold text-primary">NU</span>}
                  </div>
                  <p className={`text-[10px] font-bold tabular-nums ${current ? 'text-white' : past ? 'text-primary' : 'text-ink-soft'}`}>
                    Wk {w.week}
                  </p>
                  <p className={`text-[9px] mt-0.5 leading-tight ${current ? 'text-white/60' : past ? 'text-ink' : 'text-ink-soft'}`}>
                    {w.title}
                  </p>
                </div>
              );
        ...
        <div className="rounded-[14px] bg-surface-sunken p-4 text-center">
          <Heart size={14} className="text-tertiary mx-auto mb-1.5" />
