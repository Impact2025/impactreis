...
2. **Testdekking**: uitgebreid op 2026-09-19 met `invites.test.ts` en `onboarding.test.ts` (unit) + `e2e/login.spec.ts` en `e2e/onboarding.spec.ts` (de twee kritieke flows die hierboven als onbedekt genoemd stonden), plus `npm run test:integration` (`src/lib/__tests__/tenant-isolation.test.ts`) — bewijst tegen de echte database, met twee wegwerpbare organisaties, dat de zeven meest voorkomende lijst-routes (goals/habits/logs/weekly-goals/weekly-reviews/focus/wins) nooit data van de andere organisatie teruggeven. Draait bewust niet mee in `npm test`/CI (heeft een echte `DATABASE_URL` nodig, zie `vitest.integration.config.ts`) — handmatig draaien bij wijzigingen aan `getAuthContext()` of aan een van die zeven routes. Nog steeds geen brede route-integratietests voor de overige ~25 API-routes.
3. **Geen RLS, wel geen blinde vlek**: rij-niveau beveiliging in Postgres zelf blijft uitgesteld (zie hierboven, Neon's HTTP-driver), maar de tenant-isolatie is nu voor de meest kritieke routes geverifieerd i.p.v. aangenomen.
